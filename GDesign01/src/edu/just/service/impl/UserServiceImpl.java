...
public class UserServiceImpl implements UserService{
	...
	@Override
	public PageBean findUser(Integer pageNo, Integer pageSize) {
		// TODO Auto-generated method stub
		//计算总页数
		Long recordCount = (Long) sessionFactory.getCurrentSession().createQuery("select count(*) from User").uniqueResult();
		System.out.println(recordCount);
		List recordList = (List) sessionFactory.getCurrentSession().createQuery(" from User u order by u.id asc")
				.setFirstResult((pageNo-1) * pageSize)
				.setMaxResults(pageSize)
				.list();
		User u = (User) recordList.get(0);
